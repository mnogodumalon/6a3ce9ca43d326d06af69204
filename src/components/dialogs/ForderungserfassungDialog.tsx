...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Forderungserfassung';
...
interface ForderungserfassungDialogProps {
  open: boolean;
  onClose: () => void;
...
export function ForderungserfassungDialog({ open, onClose, onSubmit, defaultValues, recordId, schuldnerverwaltungList, enablePhotoScan = true, enablePhotoLocation = true }: ForderungserfassungDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["schuldner"]);
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, dokument_rechnung: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Forderungserfassung bearbeiten' : 'Forderungserfassung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'gezahlter_betrag': (
      <div key="gezahlter_betrag" className="space-y-1.5">
    ...
    'notizen_forderung': (
      <div key="notizen_forderung" className="space-y-1.5">
    ...
    'dokument_rechnung': (
      <div key="dokument_rechnung" className="space-y-1.5">
        <Label htmlFor="dokument_rechnung">Rechnungsdokument</Label>
        {fields.dokument_rechnung ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.dokument_rechnung.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, dokument_rechnung: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, dokument_rechnung: undefined }))}
                >
                  Entfernen
                </button>
              </div>
