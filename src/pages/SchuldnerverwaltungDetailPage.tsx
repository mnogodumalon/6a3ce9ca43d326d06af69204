...
import { formEnhancements } from '@/config/form-enhancements/Schuldnerverwaltung';
import { evalComputed } from '@/config/form-enhancements/types';

export default function SchuldnerverwaltungDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Schuldnerverwaltung | null>(null);
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        ...
        action={
          <Button variant="ghost" onClick={() => navigate('/schuldnerverwaltung')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/schuldnerverwaltung')}
      onEdit={() => setEditing(true)}
      backLabel="Zurück"
      editLabel="Bearbeiten"
    >
      <RecordHeader title={record.fields.kundennummer ?? 'Schuldnerverwaltung'} />
